...
class Grid:
    ...
    def __str__(self):
        ...
        for i in range(self.number_of_rows):
            for j in range(self.number_of_columns):
                if self.policy[i][j] == None:
                    print_statement = print_statement + str( self.policy[i][j]) + '\t'
                else:
                    print_statement = print_statement + str( self.policy[i][j])+ "\t"
...
def value_iterate(grid, noise, discount, living_reward, k):
    ...
    for i in range(grid.number_of_rows):
        for j in range(grid.number_of_columns):
            ...
            key_with_largest_value = max(policy_dict, key=policy_dict.get)
            new_grid.policy[i][j] = key_with_largest_value
